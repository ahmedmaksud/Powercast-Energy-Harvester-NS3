...
def create_single_plot(sta_data, sta_id, voltage_class):
    ...
    # Map capacitor class to actual values (ALL IN MICROFARADS)
    cap_info = {
        'A': '50000μF (50mF) Super Cap',
        'B': '2200μF Electrolytic', 
        'C': '200000μF (200mF) High Capacity'
    }
    ...
    cap_description = cap_info.get(str(cap_class), f'Class {cap_class}')
...
• Success Rate: {success_rate:.1f}%'''
    ...
    # Position info box outside the plot area (left side)
    props = dict(boxstyle='round,pad=0.5', facecolor='lightblue', alpha=0.9, edgecolor='navy')
    fig.text(0.01, 0.75, stats_text, fontsize=10, verticalalignment='top', 
             bbox=props, family='monospace', transform=fig.transFigure)
    
    # Create legend
    lines1, labels1 = ax1.get_legend_handles_labels()
    
    # Position legend on the right side, below the middle
    fig.legend(lines1, labels1, loc='center right', 
               bbox_to_anchor=(0.99, 0.4), fontsize=10)
    
    # Adjust layout to accommodate external info box and legend
    # Leave space on left for info box and right for legend
    plt.subplots_adjust(left=0.22, right=0.85, top=0.88, bottom=0.1)
...
def main():
    ...
    for sta_id in stas_to_process:
        ...
        # Generic capacitor descriptions - actual values are in the CSV from config file
        cap_info = {
            'A': 'Class A Capacitor',
            'B': 'Class B Capacitor', 
            'C': 'Class C Capacitor'
        ...
        cap_description = cap_info.get(str(cap_class), f'Class {cap_class} Capacitor')
        ...
        fig = create_single_plot(sta_data, sta_id, voltage_class)
